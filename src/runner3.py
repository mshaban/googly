...
def test_image(image_model, googly0_model, show=False):
    ...
    artifacts = ModelArtifacts(config_path=face_config_path, batch_size=1)
    face_infer_model = VinoFaceInferModel(artifacts=artifacts)
    eye_config_path = "config/eye_model_config.json"
    artifacts = ModelArtifacts(config_path=eye_config_path, batch_size=4)
    ...
    face_eyes_preds = eye_infer_model.predict(image_model, faces)
    print(len(face_eyes_preds))
